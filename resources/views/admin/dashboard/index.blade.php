                            ...
                            <li>
                                <p>
                                    ...
                                    @if($post->type == \App\Constant\PostType::Article->value)
                                        <a href="{{ route('articles.show', $post->id) }}" target="_blank">{{ $post->title }}</a>
                                    @elseif($post->type == \App\Constant\PostType::Tweet->value)
                                        <a href="{{ route('tweets.show', $post->id) }}" target="_blank">{{ Str::limit($post->body,40) }}</a>
                                    @elseif($post->type == \App\Constant\PostType::Page->value)
                                        <a href="{{ route('pages.show', $post->id) }}" target="_blank">{{ $post->title }}</a>
                                    @endif
                                </p>
                            </li>
                        @endforeach
                    ...
                    <ul class="list-unstyled">
                        ...
                        <li class="border-bottom pt-1">
                            <span class="text-muted">
                                由@if($comment->author){{ $comment->author->name }}@else{{ $comment->guest_name }}@endif发表在《@if($comment->post->type == \App\Constant\PostType::Article->value)
                                    <a href="{{ route('articles.show', $comment->post->id) }}#comment-{{ $comment->id }}" target="_blank">{{ $comment->post->title }}</a>
                                @elseif($comment->post->type == \App\Constant\PostType::Tweet->value)
                                    <a href="{{ route('tweets.show', $comment->post->id) }}#comment-{{ $comment->id }}" target="_blank">{{ Str::limit($comment->post->body,40) }}</a>
                                @elseif($comment->post->type == \App\Constant\PostType::Page->value)
                                    <a href="{{ route('pages.show', $comment->post->id) }}#comment-{{ $comment->id }}" target="_blank">{{ $comment->post->title }}</a>
                                @endif
