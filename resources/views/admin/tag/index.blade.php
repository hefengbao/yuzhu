...
@section('content')
    <div class="row">
            ...
            <div class="card card-outline card-primary">
                <div class="card-body">
                    <form action="{{ route('admin.categories.store') }}" method="post">
                        ...
                        <p class="text-muted">这将是它在站点上显示的名字。</p>
                        <div class="form-group">
                            <label for="slug" class="control-label">别名 <sup>*</sup></label>
                            <input type="text" id="slug" name="slug" class="form-control" required>
                        </div>
                        <p class="text-muted">“别名”是在URL中使用的别称，它可以令URL更美观。通常使用小写，只能包含字母，数字和连字符（-）。</p>
                        <div class="form-group">
                        ...
                        <p class="text-muted">分类和标签不同，它可以有层级关系。您可以有一个名为“音乐”的分类，在该分类下可以有名为“流行”和“古典”的子分类（完全可选）。</p>
                        <div class="form-group">
        ...
        <div class="col-md-7">
                                ...
                                <tr>
                                    <td>
                                        ...
                                        @if($category->id != 1)
                                            ...
                                            <form class="form" id="form-{{ $category->id }}" action="{{ route('admin.categories.destroy', $category->id) }}" method="post">
                                                @csrf
                                                @method('delete')
                                            </form>
                                        @endif
                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            @if($item->id != 1)
                                                ...
                                                <form class="form" id="form-{{ $item->id }}" action="{{ route('admin.categories.destroy', $item->id) }}" method="post">
                                                    @csrf
                                                    @method('delete')
                                                </form>
                                            @endif
            ...
            <p class="text-muted">删除分类不会删除分类中的文章。然而，仅隶属于已删除分类的文章将会分入默认分类<b>{{ $categories->where('id',1)->first()->name }}</b>中。默认分类不能被删除。</p>
        </div>
    </div>
@endsection
@section('script')
