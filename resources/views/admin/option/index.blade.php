                ...
                <form action="{{ route('admin.options.store') }}" class="form-horizontal" method="post">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="icp" name="icp" class="form-control" value="{{ $options['icp'] }}"
                                   aria-describedby="icpHelp">
                        </div>
                        <div class="form-group">
                            <label for="icp">搜索引擎验证 Meta</label>
                            <textarea name="site_verify_meta" id="site_verify_meta" rows="5" class="form-control"
                                      aria-describedby="siteVerifyMetaHelp">{{ $options['site_verify_meta'] ?? '' }}</textarea>
                            <div id="siteVerifyMetaHelp" class="form-text text-muted">
                                在常用的搜索引擎验证提交验证网站，可以借助平台提供的工具做一些运营分析等。<br>百度：https://ziyuan.baidu.com
                                <br>必应：https://www.bing.com/webmasters <br>谷歌：https://search.google.com/search-console
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="icp">网站分析平台接入代码</label>
                            <textarea name="site_analytics" id="site_analytics" rows="5" class="form-control"
                                      aria-describedby="siteAnalyticsHelp">{{ $options['site_analytics'] ?? '' }}</textarea>
                            <div id="siteAnalyticsHelp" class="form-text text-muted">谷歌：https://analytics.google.com
                            </div>
                        </div>
                        <div class="form-group">
