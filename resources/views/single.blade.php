                ...
                <div class="blog-post-content">
                    ...
                    <div class="text-center">
                        @if($post->user->wechatpay || $post->user->alipay)
                            <button type="button" class="btn btn-danger btn-raised" id="btn-favorite"><i
                                        class="fa fa-jpy" aria-hidden="true"></i> &nbsp;打赏&nbsp;
                            </button>
                        @endif
                    </div>
                ...
                <div class="panel-body">
                            ...
                            @foreach($comments as $key => $comment)
                                <li id="comment-{{ $comment->id }}" class="anchor">
                                    <article class="row">
                                        ...
                                        <div class="col-sm-10">
                                            <p><a href="javascript:;" id="name">{{ $comment->comment_author }}</a>&nbsp;&nbsp;<i
                                                        class="blog-post-date">{{$comment->created_at}}</i><span
                                                        class="pull-right"><a
                                                            href="javascript:void(0);">#{{ $key+1 }}</a></span></p>
                    ...
                    <div id="comment_thread">
                        <form action="{{ route('comment.store') }}" method="post" class="form-horizontal">
                            ...
                            @if(Auth::check())
                                <input type="hidden" id="comment_author" name="comment_author"
                                       value="{{ Auth::user()->name }}">
                                <input type="hidden" id="comment_author_email" name="comment_author_email"
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" class="form-control" id="comment_author"
                                               name="comment_author" placeholder="昵称" required>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label for="comment_author_email" class="col-sm-2 control-label">邮箱
                                        <sup>*</sup></label>
                                    <div class="col-sm-10">
                                        <input type="email" class="form-control" id="comment_author_email"
                                               name="comment_author_email" placeholder="邮箱地址" required>
                                    </div>
                                </div>
                            @endif
                            <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    <textarea name="comment_content" id="comment_content" class="form-control" rows="4"
                                              placeholder="请输入评论内容,支持Markdown语法"></textarea>
                                </div>
                            </div>
                            <div class="form-group">
